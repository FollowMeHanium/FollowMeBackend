...
for(let i = 30; i < 306; i ++)
{
    let photo_input = async function (id) {
        ...
        for(let j = 0; j <= times; j++)
        {
            ...
            let key = 'photo' + (j+1);
            let val = process.env.IMAGE_DB_PATH + photo_num + '.jpg';

            photo_json[key] = val;
        }
    }
    //photo_input(i);
    let grade_input = async function (id) {
	let grade = await getRandomArbitrary(1,11);
	Info.update({"grade_avg":grade},{where : {"id":id}});
    }
    grade_input(i);
